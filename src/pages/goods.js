...
class Goods extends Component {
    ...
    async componentDidMount(){
        ...
        await Axios.get(`http://localhost:1912/goods/`+id).then(res=>{
             let {data} = res;
                this.setState({
                    data,
                    id
    ...
    // }
    addTocart =() =>{
        let {src,title,text} = this.state.data[0];
        let action = {
            type:"add_to_cart",
            payload:{
                src,
        ...
        Axios.post("http://localhost:1912/goods",{
            src,
            title,
            text
        })
    ...
    render(){
        let {data,hotlist,id} = this.state;
        ...
        return <div className="goods">
            <header className="detail-header-scroll">
                ...
                <div className="header-index"><Icon onClick={()=>{this.props.history.push("/home/")}}  style={{fontSize:20}} type="home" /></div>
                <div className="header-share icon-share"><Icon style={{fontSize:20}}  type="share-alt"/></div>
            </header>
            ...
            {
                data.map((item,index)=>{
                   return(<section key={index} style={{fontSize:20}} className="detail-summary font-26">{item.text}</section>) 
                })
            }  
            ...
            <footer className="detail-footern">
                <div className="btn-fav">
                    ...
                    <Button top={900} onClick={success,this.addTocart} block={true} size="large">加入收藏</Button>
                </div>
